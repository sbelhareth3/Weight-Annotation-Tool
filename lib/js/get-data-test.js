...
client.request("Observation?" + query, {
  ...
  function(ob) {
	...
	weight_anno = weight[0]['resourceType'] + "/" + weight[0]['id']
    console.log(weight_anno)
...
var queries = new URLSearchParams();
  queries.set("patient", client.patient.id);
  queries.set("_count", 100);
  queries.set("_sort", "-date");


  client.request("/MedicationRequest?" + queries, {
      pageLimit: 0,
      flat: true
    }).then(
      function(ob) {
        ob.forEach(function(observation) {
          var med = observation.medicationCodeableConcept.text
...
function addWeightAnnotation() {
  ...
  var author = "Jimmy Hendrix"
  var annotation = author + " - " + text + " - " + time

  displayAnnotation(annotation);
  
  client.request(weight_anno).then(function(res){
    console.log(res)
    var anno = res.note
    console.log(anno)
    anno.push({authorString: author, text:annotation, time: time})
    console.log(anno)
    res.note = anno
    console.log(res)
    client.update(res)
